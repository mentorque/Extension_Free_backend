...
const ChatScreen = () => {
  ...
  const handleAsk = async () => {
    ...
    if (!jobData?.description || jobData.description.trim() === '') {
      alert('Please provide a job description to use the chat feature.');
      return;
    }
    
    if (!resumeData?.result?.formatted_resume || Object.keys(resumeData.result.formatted_resume).length === 0) {
      alert('Please upload your resume first to use the chat feature.');
      return;
    }
        ...
        <div style={contentStyle}>
          ...
          <div style={cardStyle}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              >
                {isLoading ? 'Thinking…' : isExceeded ? 'Daily limit reached' : 'Generate Answer'}
              </button>
            </div>
          </div>
