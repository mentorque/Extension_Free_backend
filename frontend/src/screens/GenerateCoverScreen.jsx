...
const GenerateCoverScreen = () => {
  ...
  const handleSave = async () => {
    ...
    if (!body.jobDescription || body.jobDescription.trim() === '') {
      alert('Please provide a job description to generate a cover letter.');
      return;
    }
    
    if (!body.resume || Object.keys(body.resume).length === 0) {
      alert('Please upload your resume first to generate a cover letter.');
      return;
    }
        ...
        <div style={contentStyle}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <button
              ...
              style={buttonStyle}
              onMouseEnter={(e) => { if (!isLoading && !isExceeded) e.target.style.backgroundColor = '#1d4ed8'; }}
              onMouseLeave={(e) => { if (!isLoading && !isExceeded) e.target.style.backgroundColor = '#2563eb'; }}
            >
              {isExceeded ? (
